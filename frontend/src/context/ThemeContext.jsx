...
export const ThemeProvider = ({ children }) => {
  const [theme, setTheme] = useState(colorMap.blue);
  const [loading, setLoading] = useState(true);
  ...
  const fetchTheme = async (tenantName) => {
    try {
      ...
      // ✅ Fetch one company directly
      const res = await fetch(`/api/companies/tenant`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
          Tenant: tenantName, // backend expects this
        },
      });

      if (!res.ok) throw new Error("Failed to fetch company theme");

      const company = await res.json(); // returns a single company
